...
export default async function OrderPage() {
  ...
  return (
    <div className="flex flex-col p-10 max-md:p-3 mb-7 gap-5  ">
      <h1 className="text-2xl max-md:text-xl ">Your Orders :</h1>
      <div className="flex flex-col items-start gap-4 ">
        {Orders.map((order: any) => (
          <div key={order._id} className="border border-blue-400/35 p-4 rounded-lg w-full">
            <div className="flex justify-start items-center gap-8 mb-4 text-lg max-md:text-[14px]">
              ...
              {order.products.map((item: any) => (
                <li key={item.product._id} className="flex gap-4 p-6 hover:bg-gray-50 hover:rounded-2xl items-center max-h-36">
                    <Image
                      ...
                      className="mr-2"
                    />
                    <div className="flex flex-col gap-2  max-md:text-[14px]">
